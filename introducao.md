# Tópicos de Inversão em Geofísica {data-background="#023b5c"}

---

## Introdução { margin="0"}

quem eu sou

quem são vocês

qual é nosso objetivo

---

# Eu {data-background="#023b5c"}

---

## Leonardo Uieda

Professor de Geofísica na [UERJ](http://www.uerj.br/)

![](img/UERJ.png)

---

Graduação em Geofísica

[IAG / USP](http://www.iag.usp.br/)

<div style="margin-top: 80px"></div>

Mestrado e Doutorado

[Observatório Nacional](http://on.br/)
<!--![](img/ON.png)-->

---

Grupo de Problemas Inversos em Geofísica

[github.com/pinga-lab](https://github.com/pinga-lab)

![](img/pinga.png)

---

* Métodos potenciais
* Modelagem e inversão

---

* Software livre

<div style="margin-top: 80px"></div>

[![](img/fatiando-logo.png)](http://www.fatiando.org)

---

## [leouieda.com](http://leouieda.com)

---

# Vocês {data-background="#023b5c"}

---

## Quem são vocês?

Curso?

Programação?
(Linguagem?)

Inversão?

---

# Curso {data-background="#023b5c"}

---

## Por que inversão?

---

# {data-background="img/inversao-e-geofisica-1.png" data-background-transition="none"}

# {data-background="img/inversao-e-geofisica-2.png" data-background-transition="none"}

# {data-background="img/inversao-e-geofisica-3.png" data-background-transition="none"}

# {data-background="img/inversao-e-geofisica-4.png" data-background-transition="none"}

---

<span style="color:#e72323">medições</span> na superfície

<i class="fa fa-arrow-down fa-3x"></i>

<span style="color:#00db74">modelo</span> da Terra

<p class="fragment" style="font-size:100pt; text-decoration:underline;">
Problema inverso
</p>

---

## Inversão na geofísica

<p class="fragment" style="font-size:50pt">
terremoto <i class="fa fa-arrow-right"></i> tomografia
</p>

<p class="fragment" style="font-size:50pt">
mag. <i class="fa fa-arrow-right"></i> deconv. Euler
</p>

<p class="fragment" style="font-size:50pt">
paleomag. <i class="fa fa-arrow-right"></i>magnetização
</p>

<p class="fragment" style="font-size:50pt">
<i class="fa fa-bolt" style="color:#fff331"></i> no Res2Dinv
</p>

---

## Objetivo = Desmitificar

---

teóricas + práticas

exemplos simples

(um pouco de [Python](http://www.python.org/))

---

## Ferramentas

* Álgebra linear
* Cálculo numérico
* Programação
* Boa vontade

---

## Online

<span style="font-size: 50pt">
[github.com/pinga-lab/inversao-unb-2014](https://github.com/pinga-lab/inversao-unb-2014)
</span>

Apresentações, material das práticas, intruções, apostila

---

## Apostila

Equações

Tutorial de matrizes

Aprofundar depois do curso

---

## Próximo tópico:

[Dados, parâmetros e problemas inversos](dados_e_parametros.html)
